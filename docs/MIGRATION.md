...
```text
Stabilize first. Stage safely. Migrate gradually. Move stateful services last.
```
...
| 1 | K3s staging/server rehearsal | cluster boots, ingress works, certs issue |
...
2. deploy K3s equivalent under a staging hostname.
...
Use K3s as a deliberate platform path, not as an emergency replacement. The Swarm/Nginx repo remains the stable current edge until each service has backup, restore, validation, and rollback proof.
